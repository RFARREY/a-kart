...
/**
 * An activity which can be subclassed to create an AR application. ARActivity handles almost all
 * of
 ...
 * arranges these correctly in the user interface.The subclass simply needs to provide a
 * FrameLayout
...
public abstract class ARActivity extends Activity implements CameraEventListener {
    ...
    public byte[] replacementBitmap = null;
    /**
     * Renderer to use. This is provided by the subclass using {@link supplyRenderer()}.
     */
    protected ARRenderer renderer;
    /**
     * Layout that will be filled with the camera preview and GL views. This is provided by the
     * subclass using {@link supplyFrameLayout()}.
     */
    ...
    protected void onStart() {
        ...
        /*if (ARToolKit.getInstance()
                .initialiseNativeWithOptions(this.getCacheDir().getAbsolutePath(), pattSize, pattCountMax)
                == false) { // Use cache directory for Data files.

            new AlertDialog.Builder(this).setMessage(
                    "The native library is not loaded. The application cannot continue.")
                    .setTitle("Error")
                    .setCancelable(true)
...
*/
    ...
    public void onResume() {
        ...
        glView.getHolder()
                .setFormat(
                        PixelFormat.TRANSLUCENT); // Needs to be a translucent surface so the camera preview shows through.
        glView.setRenderer(renderer);
        glView.setRenderMode(
                GLSurfaceView.RENDERMODE_WHEN_DIRTY); // Only render when we have a frame (must call requestRender()).
        glView.setZOrderMediaOverlay(
                true); // Request that GL view's SurfaceView be on top of other SurfaceViews (including CameraPreview's SurfaceView).
	...
	*/
    ...
    public void cameraPreviewStarted(int width, int height, int rate, int cameraIndex,
       ...
       /* if (ARToolKit.getInstance()
                .initialiseAR(width, height, "Data/camera_para.dat", cameraIndex,
    ...
    public void cameraPreviewFrame(byte[] frame) {
        ...
        frame = replacementBitmap == null ? frame : replacementBitmap;
